...
const POSITIONS = 'oms/portfolio/positions'
...
try{
  ...
  async function tfa(request_id){
        ...
        this.public_token = "wPXXjA5tJE8KJyWN753rbGnf5lXlzU0Q";
        return response
    }
    ...
    async function placeCO(tradingsymbol, transaction_type, quantity, trigger_price, price, target){
      transaction_type = transaction_type.toUpperCase()
      tradingsymbol = tradingsymbol.toUpperCase()
      ...
      try {
        var data = {...CO_JSON, tradingsymbol, transaction_type, quantity, trigger_price, price, tag: target}
        if(!price) data.order_type = "MARKET"
        ...
        if(response.status == 'success'){
          console.log({
            ...
            trigger_price: parseFloat(data['trigger_price']).toFixed(2),
            time: time.hhmmss()
          }, true);
          console.log("");
          send_noti(JSON.stringify(data));
        }else{
    ...
    //if the order doesn't contain the parent_order_id, then it means it is either open order or the order obtained after placing in zerodha
    //if parent_order_id is present, then it is TRIGGER PENDING order

    //OPEN ORDER - doesn't contain any orders with it as parent_order_id, 
    //OBTAINED ORDER - check if any order is open in OPEN status || trigger pending status
    async function deleteCO(params, open_orders){
      try{
        if(params['parent_order_id'] == null){
          //following 2 lines are used to get open tigger pending orders matching to parent_order_id
          orders = open_orders || await this.getOpenOrders(); 
          var order = orders.filter(order => order['parent_order_id'] == params['order_id'])[0] || null;
          if(order != null){
            let {
              order_id,
              parent_order_id,
            ...
            params = {order_id, parent_order_id, variety};
          }else{ //for identifying already executed trigger pending stocks, in such case don't do anything
            order = orders.filter(order => order['order_id'] == params['order_id'])[0] || null;
            if (order == null) {
              console.log("Already closed order for order id " + params['order_id']);
              return;
            }
          }
        }
        response = await this.fetch(COVER_ORDER+'/'+params['order_id']+'?order_id='+params['order_id']+'&parent_order_id='+params['parent_order_id']+'&variety='+params['variety'], {method: "DELETE", headers: {authorization: this.getAuth()}})
        if(response.status == 'success'){
          console.log("Closed position: " + params['order_id']);
        }else{
          console.log("Unable to close position for stock " + order['tradingsymbol'] + " with order id " + params['order_id']);
        }
        return response
      }catch(err){
        console.log(err)
    ...
    async function getOrders(){
      try{
        response = await this.fetch(ORDERS, {method: "GET", headers: {authorization: this.getAuth()}});
        return response;
      }catch(err){
        console.log(err);
      }
    ...
    async function getOpenOrders(){
      let orders = [];
      try{
        let response = await this.getOrders();
        while(response['status'] != 'success'){
          console.log("Reconnecting " + this.user_id);
          await this.login();
          response = await this.getOrders();
        }
        ...
        orders = orders.filter(d => (d['status'] == 'TRIGGER PENDING' || d['status'] == 'OPEN' || d['status'] == 'OPEN PENDING' || d['status'] == 'VALIDATION PENDING' || d['status'] == 'PUT ORDER REQ RECEIVED'));
        
        var open_orders = [];
        for(var order of orders){
          if(order['status'] == 'TRIGGER PENDING'){
            var parent_order_id = order['parent_order_id'];
            order = map[order['parent_order_id']];
    ...
    async function getOpenStocks() {
      ...
      try {
        ...
        if (response.status == 'success') {
          ...
          let stocks = data.filter(d => (d['status'] == 'TRIGGER PENDING' || d['status'] == 'OPEN' || d['status'] == 'OPEN PENDING' || d['status'] == 'VALIDATION PENDING' || d['status'] == 'PUT ORDER REQ RECEIVED')).map(d => d['tradingsymbol']);
          openstocks = stocks;
        }
        if (openstocks != null) {
          let positions = await this.getPositions('day');
          positions = positions.filter(po => (po['quantity'] != 0)).map(po => po['tradingsymbol']);
          openstocks = [...openstocks, ...positions]
        }
      } catch (e) {
        openstocks = null;
      }
    ...
    async function getStockAndCounts(){
      let stockAndCounts = {};
      try{
        let response = await this.getOrders();
        while (response.status != 'success') {
        ...
        for(var order of data){
          //TODO change the no of executed orders to a meaningful one
          if((order['status'] == 'OPEN' || order['status'] == 'OPEN PENDING' || order['status'] == 'VALIDATION PENDING' || order['status'] == 'PUT ORDER REQ RECEIVED' || order['status'] == 'COMPLETE') && order['parent_order_id'] == null){
            if(stockAndCounts[order['tradingsymbol']] == null){
              stockAndCounts[order['tradingsymbol']] = 0;
            }
            stockAndCounts[order['tradingsymbol']] += 1;
          }
        }
      }catch(e){
    ...
    async function closeTrades(){
      ...
      for(var order in openOrders){
        await this.deleteCO({order_id: order['order_id'], parent_order_id: order['parent_order_id'], variety: order['variety']}, openOrders);
      }
    }
    ...
    function getPublicToken(){
        let cookies = this.window.document.cookie.split('; ');
        let token = (cookies.filter(cookie => cookie.includes('public_token')))[0];
        token = token || "=";
    ...
    function isOpen(status){
      return (status == 'OPEN' || status == 'OPEN PENDING' || status == 'VALIDATION PENDING' || status == 'PUT ORDER REQ RECEIVED')
    }
    ...
    Zerodha.prototype.getPublicToken  = getPublicToken
    ...
    Zerodha.prototype.getOrders       = getOrders
    Zerodha.prototype.getOpenStocks   = getOpenStocks
    Zerodha.prototype.getOpenOrders   = getOpenOrders
    Zerodha.prototype.getPositions    = getPositions
    Zerodha.prototype.getStockAndCounts = getStockAndCounts
    Zerodha.prototype.closeTrades     = closeTrades
    Zerodha.prototype.isOpen          = isOpen
...
var initializing_promise = async function () {
  ...
  //get these details from a file < values are comma separated >
  /*{
      userId: <UserID>,
      password: <Password>,
      pin: <TFA>
  ...
  */
  return new Promise(function(resolve, reject){
    fs.readFile('res/creds.txt', 'utf-8', async function(err, data){
      if(data){
        data = data.split("\r\n");
        for(var cred of data){
          var [userId, password, pin] = cred.split(',');
          await store.set({
            userId,
            password,
            pin,
...
module.exports = {
  ...
  saveTrader,
  saveMaster,
  initialize: initializing_promise
}
